...
class Destination extends BaseController
{
    ...
    function update(){
        ...
        $destinationInfo = $this->destination_model->getDestinationInfo($id);
        $logs = ($destinationInfo->dest_log)?json_decode($destinationInfo->dest_log):array();
        $logs[] = array('date' => date('Y-m-d H:i:s'), 'userId' => $this->session->userdata('userId'));

        $this->db->where('dest_id', $id);
        $this->db->update('destination_master', array($field => $value, 'dest_log' => json_encode($logs) ));
    }
}
