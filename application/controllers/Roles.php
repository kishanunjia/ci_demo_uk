...
class Roles extends BaseController
{
    ...
    function edit($roleId = NULL)
    {
        ...
        else
        {
            ...
            $this->db->select('*');
            $this->db->from('form_list_master');
            $query = $this->db->get();
            $data['moduleList'] = $query->result_array(); //$this->config->item('moduleList');
    ...
    public function storeAccessMatrix()
    {
        ...
        foreach($modules as $module) {
            $module['total_access'] = $module['list'] = $module['create_records'] = $module['edit_records'] = $module['delete_records'] = 0;
            $singleModule = ['module'=>$module['fl_form']];
            foreach($module as $keyMod=>$valMod) {
                if(isset($postParams[$module['fl_form']][$keyMod])) {
                    $singleModule[$keyMod] = $postParams[$module['fl_form']][$keyMod] == 'on' ? 1 : $postParams[$module['fl_form']][$keyMod];
                } else {
