                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        if(!empty($moduleList))
                        {
                            ...
                            {
                                $key = array_search($record['fl_form'], array_column($roleAccessMatrix, 'fl_form'));
                                $matrix = (array) $roleAccessMatrix[$key];
                        ?>
                        <tr>
                            <td><b><?php echo $record['fl_form'] ?></b> <input type="hidden" name="access[<?= $record['fl_form'] ?>][fl_form]" value="<?php echo $record['fl_form'] ?>"  /> </td>
                            <td style="display: none;"><input type='checkbox' name='access[<?= $record['fl_form'] ?>][total_access]' <?= ($matrix['total_access'] == 1) ? 'checked':''; ?> /></td>
                            <td><input type='checkbox' name='access[<?= $record['fl_form'] ?>][list]' <?= ($matrix['list'] == 1) ? 'checked':''; ?> /></td>
                            <td><input type='checkbox' name='access[<?= $record['fl_form'] ?>][create_records]' <?= ($matrix['create_records'] == 1) ? 'checked':''; ?> /></td>
                            <td><input type='checkbox' name='access[<?= $record['fl_form'] ?>][edit_records]' <?= ($matrix['edit_records'] == 1) ? 'checked':''; ?> /></td>
                            <td><input type='checkbox' name='access[<?= $record['fl_form'] ?>][delete_records]' <?= ($matrix['delete_records'] == 1) ? 'checked':''; ?> /></td>
                        </tr>
                        <?php
                            }
                        }
