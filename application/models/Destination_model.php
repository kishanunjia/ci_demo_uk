...
class Destination_model extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from('destination_master as BaseTbl');
     ...
     */
    function destinationListing($searchText = '', $page, $segment)
    {
        $this->db->select('BaseTbl.*,tbl_users.name');
        $this->db->from('destination_master as BaseTbl');
        $this->db->join('tbl_users', 'tbl_users.userId = BaseTbl.userId', 'left');
        $this->db->order_by('BaseTbl.dest_id', 'DESC');
     ...
     */
    function addNewDestination($dataInfo)
    {
        $this->db->trans_start();
        $this->db->insert('destination_master', $dataInfo);
     ...
     * @param number $dest_id : This is factory id
     */
    function editDestination($factoryInfo, $dest_id)
    {
        $this->db->where('dest_id', $dest_id);
        $this->db->update('destination_master', $factoryInfo);
     ...
     */
    function deleteDestination($dest_id){
        
        $this->db->where('dest_id', $dest_id);
        $this->db->delete('destination_master');
